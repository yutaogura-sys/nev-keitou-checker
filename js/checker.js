...
const DrawingChecker = (() => {
  ...
  const NEV_COMMON_CHECKS = [
    ...
    { id: 'nev_scale', category: 'nev_basic_info', label: '縮尺の記載', detail: '電気系統図は配線接続の概念図であり縮尺は本来不要。表題欄に縮尺欄がある場合は「-」（ノンスケール）等の記載があればpass。縮尺欄自体がない場合もpass。実寸を示す縮尺（1/100等）が記載されている場合もpass。' },
    ...
    { id: 'nev_ground_class', category: 'nev_ground', label: '接地種別の記載', detail: '例：Ec、Ed、EC、ED等の接地種別が記載されていること。大文字・小文字の表記揺れ（Ed/ED/ed等）はいずれも有効。' },
  ...
  const NEV_CONDITIONAL_CHECKS = [
    ...
    // ⑩ デマンド制御
    { id: 'nev_demand', category: 'nev_demand', label: 'デマンド制御の記載', detail: '主幹ATの定格動作台数（100%出力可能台数）を設置台数が超える場合、デマンドコントロールの記載が必須。定格動作台数: 40AT→1台、75AT→2台、100AT→3台、125AT→4台、150AT→5台、200AT→6台、225AT→7台、250AT→8台。設置台数≦定格動作台数の場合はna。', condition: '設置台数が主幹ATの定格動作台数を超える場合' },
  ...
  const MANUAL_KISO_CHECKS = [
    ...
    { id: 'man_k_scale', category: 'man_basic', label: '縮尺欄の記載（参考）', detail: '電気系統図は概念図のため縮尺は本来不要。表題欄に縮尺欄がある場合は「A3:1/100」または「-」の記載が望ましい。縮尺欄がない場合でもna（電気系統図には適用しない要件）。', condition: '表題欄に縮尺欄がある場合（参考項目）' },
  ...
  function buildPrompt(type) {
    ...
    const typeLabel = isKiso ? '基礎充電（マンション等集合住宅向け 6kW）' : '目的地充電（商業施設・ホテル等向け 6kW/9.6kW）';
...
**複数ページの統合読取り（重要）**: 図面が複数ページにわたる場合、全ページの情報を統合して判定すること。
- 1ページ目に表題欄・全体図、2ページ目以降に詳細図・機器リスト・既設図等が分かれることがある
- メーカー名・型式・仕様等の情報が2ページ目以降にのみ記載されている場合もある
- 各チェック項目は全ページを通じて情報が見つかればpass（1ページ目になくても2ページ目以降にあればOK）
...
- 設置場所名称の記載（略称不可）
- 作成者名の記載
- 縮尺の記載（電気系統図は概念図のため縮尺は本来不要。表題欄に縮尺欄があれば「-」等の記載でpass。欄自体がなくてもpass）
...
⑦ **接地の仕様**
- 接地箇所（どこから接地に配線するか）
- 接地種別（例：Ec、Ed、EC、ED等。大文字・小文字の表記揺れはいずれも有効）
- アース線（例：IV5.5sq）
...
⑩ **デマンド制御（重要）**: 主幹ATの定格動作台数を設置台数が超える場合、デマンドコントロールの記載が必須。
  定格動作台数テーブル: 40AT→1台、75AT→2台、100AT→3台、125AT→4台、150AT→5台、200AT→6台、225AT→7台、250AT→8台。
  **判定方法**: 図面の主幹ブレーカーAT値を読み取り、充電器台数と比較。設置台数 > 定格動作台数 ならデマンド制御記載必須。記載なし=fail。設置台数 ≦ 定格動作台数 なら na。
  例: 主幹75AT(定格2台)に4台接続 → デマンド制御記載が必要 → 記載なしならfail
...
- 縮尺欄の記載（参考項目）: 電気系統図は概念図のため縮尺は本来不要。表題欄に縮尺欄がある場合は「A3:1/100」または「-」が望ましいが、欄がなくてもna
- 同時運転台数の正確性（1-10台:2台同時、11-15台:3台同時、16-20台:4台同時）
- **LBルール**: 3台以上の案件は全てLB（ロードバランシング）設計が必須。1-2台の場合はLB不要のため「na」
- **幹線ケーブル**（6kW仕様・台数別）:
  - 1-2台(LBなし): CV8sq-3C
  - 2台(LBなし): CVT22sq
  - 3-5台: CVT38sq
...
- 新設盤のメーカー名・型式の記載（例：日東工業 OR16-57C）**（条件付き）**: 特例引込・新設引込の場合のみ必須。既設キュービクルからの引込（既設引込）の場合は不要のため「na」
- **デマンドコントロール記載要否の判定（重要）**:
  6kW充電器1台あたりの定格電流は30A。主幹ATの最大負荷電流で100%出力可能な台数（定格動作台数）を以下のテーブルで判定する:
  | 主幹AT | 最大負荷電流 | 定格動作台数(100%出力) |
  | 40A | 30A | 1台 |
  | 75A | 60A | 2台 |
  | 100A | 90A | 3台 |
  | 125A | 120A | 4台 |
  ...
  | 225A | 210A | 7台 |
  | 250A | 240A | 8台 |
  **判定ロジック**: 図面から主幹ブレーカーのAT値と充電器台数を読み取り、設置台数 > 定格動作台数 の場合、デマンドコントロールの記載が必須。記載がなければ「fail」とし、findingに「主幹○○AT（定格動作台数○台）に対し設置台数○台のため、デマンドコントロール記載が必要」と具体的に記載すること。
  設置台数 ≦ 定格動作台数 の場合は「na」（不要）。
- デマンドコントロール注記の記載内容（必要な場合のみ）: 「デマンドコントロール機能により充電出力を制御する」旨の注記とLB率の記載。設置台数≦定格動作台数の場合は「na」。「デマンドコントロール機能」「ロードバランシング」「出力制御」等の表現いずれも有効
...
- 図面名称 = 「電気系統図」
- 作成者名の記載があること（記載があればpass）
- 縮尺欄の記載（参考項目）: 電気系統図は概念図のため縮尺は本来不要。表題欄に縮尺欄がある場合は「-」（ノンスケール）が望ましいが、欄がなくてもna
- 配電方法の記載
- 盤名称が配線ルート図と一致
- 電源元から充電設備までの配線種類の記載
- ブレーカー仕様・容量の全記載
- 接地の記載（接地線・接地種別・盤内接続）
- **ローバラ/デマンドコントロール注記の記載（条件付き）**:
  設置台数が主幹ATの定格動作台数を超える場合（出力制御が必要な場合）のみ必須。
  定格動作台数テーブル: 40AT→1台、75AT→2台、100AT→3台、125AT→4台、150AT→5台、200AT→6台、225AT→7台、250AT→8台。
  **判定ロジック**: 図面の主幹ブレーカーAT値を読み取り、充電器台数と比較。設置台数 > 定格動作台数 → 注記必須（なければfail）。設置台数 ≦ 定格動作台数 → 出力制御不要のため「na」。
  **許容表現**: 「ローバラ」「ロードバランシング」「デマンドコントロール機能」「出力制御」等の表現いずれも有効。
- 新設盤のメーカー名・型式の記載 **（条件付き）**: 特例引込・新設引込の場合のみ必須。既設キュービクルからの引込（既設引込）の場合は不要のため「na」
- 「各ブレーカーにおいて、必要な電気容量確保確認済み」注記
- 新設部分=赤色、既設部分=黒色
- 既設予備ブレーカー/予備ブレーカーの正しい分類表記
- 他用途配線が混在していないこと
